...
:::note
The files we're creating below can be grabbed from these links. These are purely reference and may still need to be edited to suit your environment.

- [openldap-schemas-configmap.yaml](/deployment_files/OpenLDAP/openldap-schemas-configmap.yaml)
- [openldap-ldif-configmap.yaml](/deployment_files/OpenLDAP/openldap-ldif-configmap.yaml)
- [openldap-env-configmap.yaml](/deployment_files/OpenLDAP/openldap-env-configmap.yaml)
- [openldap-admin-secret.yaml](/deployment_files/OpenLDAP/openldap-admin-secret.yaml)
- [openldap-secrets.yaml](/deployment_files/OpenLDAP/openldap-secrets.yaml)
- [openldap-deploy.yaml](/deployment_files/OpenLDAP/openldap-deploy.yaml)
- [openldap-service.yaml](/deployment_files/OpenLDAP/openldap-service.yaml)
:::

### Create the config maps
Let's create some configmaps for the ldap service

[Example here](/deployment_files/OpenLDAP/openldap-schemas-configmap.yaml)


```yaml showLineNumbers title="openldap-schemas-configmap.yaml"
kind: ConfigMap
apiVersion: v1
metadata:
...
[Example here](/deployment_files/OpenLDAP/openldap-ldif-configmap.yaml)


```yaml title="openldap-ldif-configmap.yaml"
kind: ConfigMap
apiVersion: v1
metadata:
...
```yaml title="openldap-env-configmap.yaml"
kind: ConfigMap
apiVersion: v1
metadata:
  name: openldap-env
...
```yaml title="openldap-admin-secret.yaml"
kind: Secret
apiVersion: v1
metadata:
  name: openldap
...
```yaml showLineNumbers title="openldap-secrets.yaml"
kind: Secret
apiVersion: v1
metadata:
...
```yaml title="openldap-deploy.yaml"
kind: Deployment
apiVersion: apps/v1
metadata:
...
```yaml title="openldap-service.yaml"
kind: Service
apiVersion: v1
metadata:
  name: openldap
...
:::note
The example ldif is linked here. Once again, this is purely for reference and may still need to be edited to suit your environment.

- [openldap-ldif-extra-users-configmap.yaml](/deployment_files/OpenLDAP/openldap-ldif-extra-users-configmap.yaml)
:::
